...
export default function Personel(){
    ...
    async function formSubmit(e){
        ...
        const {token} = JSON.parse(window.localStorage.getItem("userInformation")|| '{}');
            const data = await axios.post('http://localhost:9090/multiPart/save/', {
    ...
    const authoritiesEditor = (options)=>{
        ...
        return(
            ...
            <ul style={{listStyleType:'none'}}>{
                person[options.rowIndex].authority.map(s=>{
                    return (<div id={s} key={s} style={{display:'flex'}}><AiOutlineClose onClick={(e)=>{
